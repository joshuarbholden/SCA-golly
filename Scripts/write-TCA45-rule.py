...
def writegollyrule (turningrule, crossingrule, brickstring):
    ...
    filenamestring = g.getdir('rules') + "TCA45" + brickstring + "-T%dC%d.rule" % (turningrule, crossingrule)
    outfile = open(filenamestring, 'w')
    print("@RULE TCA45" + brickstring + "-T%dC%d\n" % (turningrule, crossingrule), file=outfile)
    print("@TABLE\n", file=outfile)
...
g.setrule("TCA45" + brickstring + "-T%dC%d:T%d,1000000+%d" % (turningrule, crossingrule, width, width))
g.show('Created '+"TCA45" + brickstring + "-T%dC%d.rule" % (turningrule, crossingrule)+' and selected that rule with bounded grid T%d,1000000+%d.' % (width,width))
