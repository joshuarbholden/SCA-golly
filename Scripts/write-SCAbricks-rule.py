...
def writegollyrule (turningrule, crossingrule):
    ...
    filenamestring = g.getdir('rules') + "SCAbricks-T%dC%d.rule" % (turningrule, crossingrule)
    outfile = open(filenamestring, 'w')
    print("@RULE SCAbricks-T%dC%d\n" % (turningrule, crossingrule), file=outfile)
    print("@TABLE\n", file=outfile)
...
g.setrule("SCAbricks-T%dC%d" % (turningrule, crossingrule))
g.show('Created '+"SCAbricks-T%dC%d.rule" % (turningrule, crossingrule)+' and selected that rule.')
